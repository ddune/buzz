...
The request-evaluation turn runs in a fresh restricted session. Unknown MCP servers, general shell execution, and file mutation are unavailable; the bundled MCP permits inspection plus only the constrained `buzz jobs accept` or `buzz jobs reject` shell command. The target's matching acceptance or rejection rotates that restricted session away. ACP-launched MCP servers mark the constrained CLI environment so a successfully persisted decision call does not return control to the model; if lifecycle observation is delayed, the tool call remains pending and therefore fails closed. Acceptance triggers immediate reconciliation, but executable continuation dispatch remains behind the normal per-channel in-flight gate: the decided evaluation turn is cancelled and dropped, and accepted work receives a new unrestricted turn only after generation one has both a persisted `runnable` event and a persisted `claim` event. An evaluation that ends without a matching durable decision retains normal retry behavior. The request-evaluation turn is never itself an execution attempt.
