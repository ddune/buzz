...
The request-evaluation turn ends when the target's matching acceptance or rejection is observed. ACP-launched MCP servers mark the constrained CLI environment so a successfully persisted `buzz jobs accept` or `buzz jobs reject` call does not return control to the model; the matching lifecycle event cancels the evaluation turn. If lifecycle observation is delayed, the tool call remains pending and therefore fails closed. Acceptance triggers immediate reconciliation, but executable continuation dispatch remains behind the normal per-channel in-flight gate: the evaluation turn is cancelled and dropped, and accepted work receives a new turn only after generation one has both a persisted `runnable` event and a persisted `claim` event. The request-evaluation turn is never itself an execution attempt and is not requeued after its decision.
